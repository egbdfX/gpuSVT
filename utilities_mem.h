...
// Private implementation.
	...
	void mem_clear_contents(Mem *mem, Error *status);
	...
	void *mem_gpu_buffer(Mem *mem, Error *status);

	// Returns a pointer to the GPU buffer, if memory is on the GPU.
	const void *mem_gpu_buffer_const(const Mem *mem, Error *status);
	...
	void mem_random_fill(Mem *mem, Error *status);
	...
	int32_t mem_assert_type(Mem* A, Mem* B, Error* status);

	int32_t mem_assert_location(Mem* A, Mem* B, Error* status);

	int32_t mem_assert_nElements(Mem* A, Mem* B, Error* status);

	int32_t mem_assert_null(Mem* A, Error* status);

	// Private function.
	static void mem_alloc(Mem *mem, Error *status) {
